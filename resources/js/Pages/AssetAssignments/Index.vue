...
function applyFilter() {
    router.get(route('asset-assignments.index'), { status: selectedStatus.value }, {
...
<template>
    ...
    <AuthenticatedLayout>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-0.5">
                        {{ isAdmin ? 'Who currently holds what asset' : 'Your current and past asset assignments' }}
                    </p>
                </div>
            </div>
        </template>
        ...
        <div class="max-w-7xl mx-auto space-y-5">
            ...
            <!-- Filter bar -->
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4 flex gap-2">
                <button
                    v-for="opt in ['', 'assigned', 'returned']" :key="opt"
            ...
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-100 bg-gray-50">
                                ...
                                <th v-if="isAdmin" class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Employee</th>
                                <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Assigned</th>
                                <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Returned</th>
                                <th class="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Status</th>
                                <th class="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase">Action</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            <tr v-for="a in assignments.data" :key="a.id" class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-semibold text-gray-900">{{ a.asset?.model_name }}</p>
                                            <p class="text-xs text-gray-400">{{ a.asset?.category?.name ?? 'Uncategorized' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td v-if="isAdmin" class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <img :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(a.user?.name ?? 'U')}&color=ffffff&background=DC143C&size=28`"
                                            class="w-7 h-7 rounded-full" />
                                        <div>
                                            <p class="text-sm font-medium text-gray-900">{{ a.user?.name }}</p>
                                            <p class="text-xs text-gray-400">{{ a.user?.email }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-gray-500 text-xs">{{ fmt(a.assigned_date) }}</td>
                                <td class="px-6 py-4 text-gray-500 text-xs">{{ a.return_date ? fmt(a.return_date) : '—' }}</td>
                                <td class="px-6 py-4">
                                    <span :class="a.status === 'assigned' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'"
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <button
                                            ...
                                            <fa-icon icon="rotate-left" class="w-3.5 h-3.5" />
                                            Return
                                        </button>
                                    </div>
                                </td>
                ...
                <div v-else class="px-6 py-14 text-center">
                    ...
                    <p class="text-sm font-semibold text-gray-500 mb-1">No assignments found</p>
                    <p class="text-xs text-gray-400">
                        {{ isAdmin ? 'No assets have been assigned yet.' : 'You have no asset assignments.' }}
                    </p>
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ selectedAssignment?.asset?.model_name }}</p>
                    </div>
                </div>
                <p class="text-sm text-gray-700 mb-6">
                    Confirm returning this asset. It will become available for others to request.
                </p>
